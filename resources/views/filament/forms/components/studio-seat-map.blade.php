@php
    ...
    $studioSeats = $studio->seats()->get()->keyBy(function ($seat) {
        return $seat->row . '-' . $seat->column;
    });

    // Calcular estadísticas
    $totalSeats = $studio->seats()->count();
    $activeSeats = $studio->seats()->where('is_active', true)->count();
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="text-sm text-gray-500 dark:text-gray-400">
                {{ $rows }}x{{ $columns }} ({{ $totalPositions }} posiciones)
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-3 text-center">
                <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $activeSeats }}</div>
            ...
            <div class="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-3 text-center">
                <div class="text-2xl font-bold text-orange-600 dark:text-orange-400">{{ $inactiveSeats }}</div>
                <div class="text-sm text-orange-600 dark:text-orange-400">Inactivos</div>
            </div>
            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center">
                <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $totalSeats }}</div>
            ...
            <div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-3 text-center">
                <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ $studio->capacity_per_seat ?? $studio->max_capacity ?? 0 }}</div>
                <div class="text-sm text-purple-600 dark:text-purple-400">Capacidad</div>
            </div>
        </div>

        {{-- Información del direccionamiento --}}
        @if($studio->addressing)
            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 mb-4">
                <div class="flex items-center justify-center gap-2 text-sm text-blue-700 dark:text-blue-300">
                    📍 <strong>Direccionamiento:</strong> {{ ucfirst(str_replace('_', ' ', $studio->addressing)) }}
                </div>
            </div>
        @endif
        ...
        <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <div class="flex flex-wrap gap-4 text-sm">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-gray-600 dark:text-gray-400">Posición Vacía</span>
                </div>
            </div>
        </div>
        ...
        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            <div class="flex gap-2">
                <a href="{{ \App\Filament\Resources\StudioResource::getUrl('edit', ['record' => $studio]) }}"
                   class="inline-flex items-center px-3 py-2 bg-blue-500 hover:bg-blue-600 text-white text-sm rounded-md transition-colors">
                    🔧 Editar Sala
                </a>
                <a href="{{ \App\Filament\Resources\StudioResource::getUrl('manage-seats', ['record' => $studio]) }}"
